...
############################################################################################################
    ...
    def finalCheck(self):
        ...
        for i in range(9):
            for j in range(9):
                if self.check(self.lst[i][j],i,j):
                    count+=1
        if count==81:
            return True
    ...
    def insert(self,n,r,c): # this function takes the user input values to the user input grid location and places it there if the parameters satisfy sudoku rules
        if r>=0 and c>=0 and r<9 and c<9 and self.check(n,r,c):
            self.lst[r][c] = n
            self.remaining -= 1
        else:
            print("The move you are trying to make is invalid!")
        if self.finalCheck(): # after insertion, if there are no available spaces on the grid, the screen is cleared and the entire grid is redisplyed and the winning message is displayed
            os.system('cls || clear')
            self.display()
...
def main():
    ...
    while ccnt<b.remain(): # The main runs until there are no free spaces in the grid
        ...
        if x == -1: break # Game terminating condition
            ...
            b.display()
        else: # Any number between 1-9 and a valid row and column number will be accepted by this function
